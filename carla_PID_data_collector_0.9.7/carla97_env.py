...
class Carla97_Env:
    ...
    def reset(self, scene_id, pose_id):
        ...
        self.ego_car = EgoCar(self.world, self.client, self.config)
...
def main():
    ...
    for scene in range(Scene_num):
        ...
        for episode in range(120):
            ...
            sensors, measures = env.reset(scene, pose)
            for i in range(2000):
                print('=========== Scene %d, Episode %d, Pose %d, Frame %d : =========' % (scene, episode, pose, i))

                sensor_dict, measure_dict, done, timestamp = env.step()
            ...
            env.close()
            sensors_list, measures_list, task_type_list, lane_type_list = np.array(sensors_list), np.array(measures_list), np.array(task_type_list), np.array(env.lane_type)
            write_hdf5(file_name, group_name, sensors_list, measures_list, task_type_list, lane_type_list, env.center)
