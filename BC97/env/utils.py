...
def get_closet_wp(path, cur_loc, debug, draw_dis):
    ...
    if draw_dis:
        ...
        debug.draw_line(cur_loc, temp_loc, thickness=0.5, color=carla.Color(255, 0, 0), life_time=0.2, persistent_lines=False)

    return path[min_ind][0], path[min_ind+1][0], math.sqrt(min_dis)
...
def pre_process_lidar(points, lidar_dim):
    ...
    for point in points:
        ...
        limit = lidar_dim % 360
        if rel_deg >= limit and rel_dis < lidar_feature[rel_deg-limit]:
...
def render_LIDAR(points, lidar_transform, debug):
    for point in points:
        ...
        debug.draw_point(point_loc, 0.1, carla.Color(255, 162, 0), 1, False)
