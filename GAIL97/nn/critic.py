        ...
        with tf.variable_scope(scope):
            ...
            for i in self.branch_list:
                with tf.variable_scope("Branch_" + str(i)):
                    fci1 = tf.layers.dense(cat1, units=256, activation=tf.nn.relu, kernel_initializer=initializer)
                    dpi1 = tf.nn.dropout(fci1, self.dropout[0])

                    outputs[i] = tf.layers.dense(dpi1, units=1, kernel_initializer=initializer) # shouldn't take activation
