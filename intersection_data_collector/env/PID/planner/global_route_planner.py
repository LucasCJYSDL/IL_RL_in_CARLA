...
"""
This module provides implementation for GlobalRoutePlanner.
"""
...
class GlobalRoutePlanner(object):
    ...
    def trace_route(self, origin, destination, debug=False):
        """
        ...
        trace_statistics = {'VOID':0,'LEFT':0,'RIGHT':0,'STRAIGHT':0, 'LANEFOLLOW':0, 'CHANGELANELEFT':0, 'CHANGELANERIGHT':0}
        for wp, option in route_trace[:-8]:
            if option == RoadOption.VOID:
                trace_statistics['VOID'] += 1
            if option == RoadOption.LEFT:
                trace_statistics['LEFT'] += 1
            if option == RoadOption.RIGHT:
                trace_statistics['RIGHT'] += 1
            if option == RoadOption.STRAIGHT:
                trace_statistics['STRAIGHT'] += 1
            if option == RoadOption.LANEFOLLOW:
                trace_statistics['LANEFOLLOW'] += 1
            if option == RoadOption.CHANGELANERIGHT:
                trace_statistics['CHANGELANERIGHT'] += 1
            if option == RoadOption.CHANGELANELEFT:
                trace_statistics['CHANGELANELEFT'] += 1
        
        s = sum([val for key, val in trace_statistics.items()])
        #p = [val / s for key, val in trace_statistics.items()]

        if debug:
            print("===========================RoadOption Statistics====================================")
            #print(trace_statistics)
            #print(p)
            for key,val in trace_statistics.items():
                print("%15s\t\t%d\t\t%f" % (key,val, val /s))
            print("====================================================================================")
